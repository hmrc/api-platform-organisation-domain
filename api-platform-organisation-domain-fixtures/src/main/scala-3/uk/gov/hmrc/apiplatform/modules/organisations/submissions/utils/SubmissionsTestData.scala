...
trait ProgressTestDataHelper extends FixedClock {
  ...
  extension (submission: Submission) {
    ...
    def withCompletedProgress(): ExtendedSubmission =
      ExtendedSubmission(submission, allQuestionnaireIds.map(_ -> completedQuestionnaireProgress).toList.toMap)

    def withNotStartedProgress(): ExtendedSubmission =
      ExtendedSubmission(submission, allQuestionnaireIds.map(_ -> notStartedQuestionnaireProgress).toList.toMap)

    def withNotApplicableProgress(): ExtendedSubmission =
      ExtendedSubmission(submission, allQuestionnaireIds.map(_ -> notApplicableQuestionnaireProgress).toList.toMap)

    def withSubmittedProgress(): ExtendedSubmission =
      ExtendedSubmission(Submission.submit(instant, "bob@example.com")(submission), allQuestionnaireIds.map(_ -> completedQuestionnaireProgress).toList.toMap)
...
trait SubmissionsTestData extends QuestionBuilder with QuestionnaireTestData with ProgressTestDataHelper with StatusTestDataHelper with FixedClock {
  ...
  private def buildAnsweredSubmission(fullyAnswered: Boolean)(submission: Submission): Submission = {
    ...
    def passAnswer(question: Question): ActualAnswer = {
      question match {
        case Question.TextQuestion(id, wording, statement, _, _, _, _, absence, _, _, _)                                      => ActualAnswer.TextAnswer("some random text")
        case Question.ChooseOneOfQuestion(id, wording, statement, _, _, _, marking, absence, _, _, _)                         => ActualAnswer.SingleChoiceAnswer(marking.filter {
            case (_, Mark.Pass) => true; case _ => false
          }.head._1.value)
        case Question.MultiChoiceQuestion(id, wording, statement, _, _, _, marking, absence, _, _, _)                         => ActualAnswer.MultipleChoiceAnswer(Set(marking.filter {
            case (_, Mark.Pass) => true; case _ => false
          }.head._1.value))
        case Question.AcknowledgementOnly(id, wording, statement, _, _)                                                       => ActualAnswer.AcknowledgedAnswer
        case Question.ForwardToQuestion(id, forwardToQuestionId, wording, statement, _, _)                                    => ActualAnswer.AcknowledgedAnswer
        case Question.DateQuestion(_, _, _, _, _, _, _, _, _, _)                                                              => ActualAnswer.DateAnswer(now.toLocalDate)
        case Question.AddressQuestion(_, _, _, _, _, _, _, _, _, _)                                                           => ActualAnswer.AddressAnswer(address)
        case Question.InternationalAddressQuestion(_, _, _, _, _, _, _, _, _, _)                                              => ActualAnswer.InternationalAddressAnswer(intlAddress)
        case Question.NameQuestion(_, _, _, _, _, _, _, _, _, _)                                                              => ActualAnswer.NameAnswer(fullName)
        case Question.ConfirmNameQuestion(_, _, _, _, _, _, _, _, _, _)                                                       => ActualAnswer.ConfirmNameAnswer(confirmFullName)
        case Question.CompanyNumberQuestion(_, _, _, _, _, _, _, _, _, _)                                                     => ActualAnswer.CompanyNumberAnswer("12345678")
        case Question.AttachmentQuestion(_, _, _, _, _, _, _, _, _, _)                                                        => ActualAnswer.AttachmentAnswer(attachment)
        case Question.YesNoQuestion(id, wording, statement, _, _, _, yesMarking, noMarking, absence, _, _, _)                 =>
          if (yesMarking == Mark.Pass) ActualAnswer.SingleChoiceAnswer("Yes") else ActualAnswer.SingleChoiceAnswer("No")
        case Question.ConfirmCompanyNameQuestion(id, wording, statement, _, _, _, yesMarking, noMarking, absence, _, _, _)    =>
          if (yesMarking == Mark.Pass) ActualAnswer.SingleChoiceAnswer("Yes") else ActualAnswer.SingleChoiceAnswer("No")
        case Question.ConfirmCompanyAddressQuestion(id, wording, statement, _, _, _, yesMarking, noMarking, absence, _, _, _) =>
          if (yesMarking == Mark.Pass) ActualAnswer.SingleChoiceAnswer("Yes") else ActualAnswer.SingleChoiceAnswer("No")
      }
    }
...
trait AnsweringQuestionsHelper extends FixedClock {
  ...
  def answerForQuestion(desiredMark: Mark)(question: Question): Map[Question.Id, Option[ActualAnswer]] = {
    val answers: List[Option[ActualAnswer]] = question match {
      ...
      case Question.YesNoQuestion(id, _, _, _, _, _, yesMarking, noMarking, absence, _, _, _) =>
      ...
      case Question.ConfirmCompanyNameQuestion(id, _, _, _, _, _, yesMarking, noMarking, absence, _, _, _) =>
        (if (yesMarking == desiredMark) Some(ActualAnswer.SingleChoiceAnswer("Yes")) else None) ::
          (if (noMarking == desiredMark) Some(ActualAnswer.SingleChoiceAnswer("No")) else None) ::
          (absence.flatMap(a => if (a._2 == desiredMark) Some(ActualAnswer.NoAnswer) else None)) ::
          List.empty[Option[ActualAnswer]]

      case Question.ConfirmCompanyAddressQuestion(id, _, _, _, _, _, yesMarking, noMarking, absence, _, _, _) =>
        (if (yesMarking == desiredMark) Some(ActualAnswer.SingleChoiceAnswer("Yes")) else None) ::
          (if (noMarking == desiredMark) Some(ActualAnswer.SingleChoiceAnswer("No")) else None) ::
          (absence.flatMap(a => if (a._2 == desiredMark) Some(ActualAnswer.NoAnswer) else None)) ::
          List.empty[Option[ActualAnswer]]

      case Question.ChooseOneOfQuestion(id, _, _, _, _, _, marking, absence, _, _, _) => {
        marking.map {
      ...
      case Question.TextQuestion(id, _, _, _, _, _, _, absence, _, _, _) =>
      ...
      case Question.AcknowledgementOnly(id, _, _, _, _)                        => List(Some(ActualAnswer.AcknowledgedAnswer))
      case Question.ForwardToQuestion(id, _, _, _, _, _)                       => List(Some(ActualAnswer.AcknowledgedAnswer))
      case Question.DateQuestion(_, _, _, _, _, _, _, _, _, _)                 => List(Some(ActualAnswer.DateAnswer(now.toLocalDate)))
      case Question.AddressQuestion(_, _, _, _, _, _, _, _, _, _)              => List(Some(ActualAnswer.AddressAnswer(address)))
      case Question.InternationalAddressQuestion(_, _, _, _, _, _, _, _, _, _) => List(Some(ActualAnswer.InternationalAddressAnswer(intlAddress)))
      case Question.NameQuestion(_, _, _, _, _, _, _, _, _, _)                 => List(Some(ActualAnswer.NameAnswer(fullName)))
      case Question.ConfirmNameQuestion(_, _, _, _, _, _, _, _, _, _)          => List(Some(ActualAnswer.ConfirmNameAnswer(confirmFullName)))
      case Question.CompanyNumberQuestion(_, _, _, _, _, _, _, _, _, _)        => List(Some(ActualAnswer.CompanyNumberAnswer("12345678")))
      case Question.AttachmentQuestion(_, _, _, _, _, _, _, _, _, _)           => List(Some(ActualAnswer.AttachmentAnswer(attachment)))

      case Question.MultiChoiceQuestion(id, _, _, _, _, _, marking, absence, _, _, _) =>
        marking.map {
